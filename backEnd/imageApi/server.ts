...
export class ImageServer {
  ...
  createServer = () => {
    ...
    return {
      server: this.server, httpServer: this.httpServer, repository: this.repository
    }
  }
}
const { httpServer, server, repository } = new ImageServer(parseInt(`${process.env.PORT}`), `${process.env.NODE_ENV}`, `${process.env.COOKIE_SECRET}`).createServer()
